...
export function Editor() {
    ...
    let [tags, setTags] = React.useState<string[]>(dataset.caption.content.split(','));
    ...
    async function onSaveCaption() {
      const fileHandle = await dataset.dirHandle.getFileHandle(dataset.caption.name, {create: true,});
    ...
    return(
        <>
          <Box sx={{maxHeight:'100vh', backgroundColor:"secondary.main"}}>
            ...
            <Grid container spacing={1}>
                ...
                <Card>
                  <ImageList sx={{ maxHeight: '80vh'}} cols={2} gap={4} >
                    {datasets.map((dataset, index) => (
                      <ImageListItem key={dataset.image.uri}
                          sx={{
                            '&:hover': {
              ...
              <Grid xs={10}>
                <Card>
                  <CardMedia component='img' image={dataset.image.uri}
            ...
            <Box justifyContent='center' alignContent='center' display='flex'>
              <Pagination count={datasets.length} page={pageIndex+1} onChange={pageChenger}/>
            </Box>
          </Box>
          <Snackbar
            open={onSaveSuccess}
